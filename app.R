...
# Reactive value for the main dataset
H_main_data <- reactiveVal(NULL)
# Reactive for column names of the main dataset
H_column_names_reactive <- reactive({
    if (!is.null(H_main_data()) && is.data.frame(H_main_data())) {
        return(names(H_main_data()))
    }
    return(character(0))
})
...
source(file.path("modules", "agreement", "agreement_ui.R"), local = TRUE)
...
server <- function(input, output, session) {
  ...
  source(file.path("modules", "brief_summary", "brief_summary_server.R"), local = TRUE)
  source(file.path("modules", "age_pyramid", "age_pyramid_logic.R"), local = TRUE)
  source(file.path("modules", "age_pyramid", "age_pyramid_server.R"), local = TRUE)
  source(file.path("modules", "agreement", "agreement_logic.R"), local = TRUE)
  source(file.path("modules", "agreement", "agreement_server.R"), local = TRUE)

  brief_summary_server("brief_summary_ns", H_main_data, H_column_names_reactive)
  age_pyramid_server("age_pyramid_ns", H_main_data, H_column_names_reactive)
  agreement_server("agreement_ns", H_main_data, H_column_names_reactive)
  # Module server functions will be called here
  # Commenting out server logic for removed UI tabs
